...
var generateRandomGoods = function (count) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    var nutrition_facts = {};
    nutrition_facts.sugar = getRandomNumber(0, 100) < 50;
    nutrition_facts.energy = getRandomNumber(70, 500);
    ...
    CONTENTS.forEach(function (content) {
      ...
      if (getRandomNumber(0, 100) < 50) { contents += content; contentCount++; }
    })
    if (contentCount === 0) contents += getRandomElement(CONTENTS);

    nutrition_facts.contents = contents;
    newGood.nutrition_facts = nutrition_facts;
    goods.push(newGood);
  }
  return goods;
...
var renderCard = function (template, good) {
  ...
  if (good.amount > 5) { card.classList.add('card--in-stock'); } else
    if (good.amount <= 0) { card.classList.add('card--soon'); } else { card.classList.add('card--little'); }

  var stars = card.querySelector('.stars__rating');
  stars.classList.forEach(
    function (starsClass) {
  ...
  card.querySelector('.card__characteristic').textContent = (good.nutrition_facts.sugar ? 'Содержит сахар. ' : 'Без сахара. ') + good.nutrition_facts.energy + ' ккал';
  card.querySelector('.card__composition-list').textContent = good.nutrition_facts.contents;
